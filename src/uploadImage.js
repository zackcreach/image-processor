...
module.exports.handler = (event, context, callback) => {
  ...
  let extension = /^\.\w*$/.test(query.extension)
    ? query.extension
    : typeof query.extension != "undefined"
    ? `.${query.extension}`
    : ".undefined";
  ...
  if (!content_type)
    return callback(null, {
      statusCode: 400,
      body: JSON.stringify({
        message: "please provide a content_type and extension"
      })
    });
  s3.getSignedUrl(
    "putObject",
    generateUploadParams(content_type, extension, key),
    function(error, url) {
      if (error)
        return callback(null, {
        ...
        callback(null, { statusCode: 200, body: JSON.stringify({ url: url }) });
    }
  );
};
