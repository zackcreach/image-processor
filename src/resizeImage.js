  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      gm.stream(file_type, (err, stdout, stderr) => {
        ...
        stdout.on("error", err =>
          reject({ statusCode: 500, body: `{"message":"${err.message}"}` })
        );
        stderr.on("data", data =>
          reject({ statusCode: 500, body: `{"message":"${data}"}` })
        );
        stdout
          .pipe(target_stream)
...
function formatEvent(event) {
  return new Promise((resolve, reject) => {
    ...
    Object.keys(EVENT_PARAMS).forEach(k => {
      if (!event[k] && EVENT_PARAMS[k].required)
        return reject({
          statusCode: 400,
          body: `{"message": "${k} required"}`
        });
      else if (typeof event[k] !== EVENT_PARAMS[k].type)
        return reject({
          statusCode: 400,
          body: `{"message": "${k} should be of type ${EVENT_PARAMS[k].type}"}`
        });
      job[k] = event[k] || EVENT_PARAMS.default;
    });
    Object.keys(job.resize_options).forEach(k => {
      if (GM_KEYS.indexOf(k) == -1) {
        delete job.resize_options[k];
      }
    });
    return resolve(job);
  });
