function parseQueryParameters(query) {
  ...
  if (
    ...
    format.resize.width =
      query.w && parseInt(query.w) !== NaN ? parseInt(query.w) : null;
    format.resize.height =
      query.h && parseInt(query.h) !== NaN ? parseInt(query.h) : null;
    if (format.resize.height !== null && format.resize.width !== null)
      format.crop = {
        width: format.resize.width,
        height: format.resize.height
      };
  }
